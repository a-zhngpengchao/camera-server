...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户表（App 账号）';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户-设备关联表（预留）';
...
CREATE TABLE `user_auth` (
  ...
  `union_id`     varchar(128)          DEFAULT NULL COMMENT '微信 unionid 等（可选）',
...
-- 7. 用户 Token 表：user_token（管理 refresh_token 等）
DROP TABLE IF EXISTS `user_token`;
CREATE TABLE `user_token` (
  `id`            bigint       NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `user_id`       bigint       NOT NULL COMMENT 'user.id',
  `access_token`  varchar(255) NOT NULL COMMENT '访问 token（可选持久化）',
...
CREATE TABLE `payment_order` (
  ...
  `status`       varchar(20)          DEFAULT 'pending' COMMENT 'pending/paid/failed/cancelled/refunded',
  `payment_method` varchar(32)        DEFAULT NULL COMMENT 'wechat/paypal/apple',
...
CREATE TABLE `wifi_history` (
  ...
  `security`    varchar(32)           DEFAULT NULL COMMENT 'WPA2 等',
...
CREATE TABLE `device_binding` (
  ...
  `wifi_password` varchar(128)       DEFAULT NULL COMMENT 'WiFi 密码（如需可加密）',
